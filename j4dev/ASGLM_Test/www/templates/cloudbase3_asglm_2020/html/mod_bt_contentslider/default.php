...
<div id="btcontentslider<?php echo $module->id; ?>" style="display:none;width:<?php echo $moduleWidthWrapper;?>" class="bt-cs<?php echo $moduleclass_sfx? ' bt-cs'.$params->get('moduleclass_sfx'):'';?>">
	...
	<?php foreach( $pages as $key => $list ): ?>
		...
		<?php endif; ?>
		<?php if (strtotime($row->publish_up) > strtotime(JFactory::getDate())) : ?>
			<span class="label label-warning"><?php echo JText::_('JNOTPUBLISHEDYET'); ?></span>
		<?php endif; ?>
		<?php if ((strtotime($row->publish_down) < strtotime(JFactory::getDate())) && $row->publish_down != JFactory::getDbo()->getNullDate()) : ?>
...
{ 
	echo '<div>'.JText::_('NO_RESULT').'</div>'; 
} ?>