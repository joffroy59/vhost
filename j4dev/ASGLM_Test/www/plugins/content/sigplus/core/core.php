...
class SigPlusNovoFlickrGallery extends SigPlusNovoXMLGallery {
	...
	*/
	public function populate($url, $folderparams) {
		...
		if ($sxml->err) {
			SigPlusNovoLogging::appendError('Unable to retrieve Flickr Web Album with user ID <code>'.htmlspecialchars($user_id).'</code> and photo set ID <code>'.htmlspecialchars($photoset_id).'</code>; query to Flickr API fails with code <code>'.htmlspecialchars($sxml->err['code']).'</code> and message: <em>'.htmlspecialchars($sxml->err['msg']).'</em>');
			return false;
		}
...
class SigPlusNovoCore {
	...
	public function downloadImage($imagesource) {
		...
		$db->setQuery(
			...
			'WHERE'.PHP_EOL.
				...
				'i.'.$db->quoteName('imageid').' = '.$imageid.$depthcond
		);
		$row = $db->loadRow();
		if (!$row) {
	...
	*/
	public function getGalleryHTML($imagesource, &$galleryid) {
		...
		// use appropriate SQL syntax depending on SQL server type
		switch ($db->getServerType()) {
			case 'mysql':
				$top1 = '';
				$limit1 = 'LIMIT 1';
	...
	/**
	* Replaces Joomla article ID references in a "labels.txt" entry.
	*
	* Looks for occurrences of <a data-article-id="...">...</a> and adds a "href" attribute to these anchors.
	* The URL is generated by Joomla's URL router, given the article ID; adding item ID and menu ID as necessary.
	*/
	private static function replaceRoutedURLs($html) {
		if (empty($html) || strpos($html, 'data-article-id=') === false) {
			return $html;
		}
		...
		try {
			$changed = false;
			$dom = new DomDocument();
			$dom->loadHTML("<div>{$html}</div>", LIBXML_HTML_NOIMPLIED | LIBXML_HTML_NODEFDTD);  // document root must be a single element
			$html_errors = libxml_get_errors();
			if (!empty($html_errors)) {  // ignore ill-formed HTML
				return $html;
			}
			...
			foreach ($links as $link) {
				...
				if (empty($href)) {  // do not overwrite existing "href" attribute value
					$article_url = Route::_("index.php?option=com_content&view=article&id={$article_id}");
