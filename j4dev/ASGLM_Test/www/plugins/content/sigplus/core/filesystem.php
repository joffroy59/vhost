...
class fsx_windows implements fsx_functions {
	...
	*/
	...
	public function imagecreatefromwebp($file) {
		if (!function_exists('imagecreatefromwebp')) {  // *.webp support is optional
			return false;
		}
	...
	public function imagewebp($image, $file, $quality) {
		if (!function_exists('imagewebp')) {  // *.webp support is optional
			return false;
		}
...
class fsx_unix implements fsx_functions {
	...
	*/
	...
	public function imagecreatefromwebp($file) {
		...
		return imagecreatefromwebp($file);  // no special wrapper is required for UNIX-style operating systems
	}
...
class fsx {
	...
	*/
	...
	public static function imagecreatefromwebp($file) {
		return self::$instance->imagecreatefromwebp($file);
	}
	...
	public static function imagewebp($image, $file, $quality) {
		return self::$instance->imagewebp($image, $file, $quality);
	}
...
function walkdir($path, array $exclude = array(), $depth = 0, $callback = null, $ancestors = null) {
	...
	foreach ($files as $key => $file) {
		if (is_hidden_file($file) || in_array($file, $exclude)) {  // skip hidden files and excluded entries
			unset($files[$key]);
		}
	}
...
/**
* Filters files, skipping those that are hidden.
