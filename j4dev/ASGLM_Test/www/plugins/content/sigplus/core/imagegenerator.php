...
class SigPlusNovoImageLibraryGD extends SigPlusNovoImageLibrary {
	...
	private static function imageFromFile($imagepath, &$orientation = null) {
		...
		switch ($ext) {
			...
			case 'png':
				...
				break;
			case 'webp':
				$image = fsx::imagecreatefromwebp($imagepath);
				break;
			default:
	...
	* @param int $quality Quality measure between 0 and 100 for JPEG and WEBP compression.
	*/
	private static function imageToFile($imagepath, $image, $quality) {
		...
		switch ($ext) {
			...
			case 'webp':
				return fsx::imagewebp($image, $imagepath, $quality);
			default:
	...
	*/
	...
	private function GIFGetByte($len) {
		...
		for ($i = 0; $i < $len; $i++) {
			...
			$this->GIF_buffer[] = ord($this->GIF_stream[$this->GIF_bfseek++]);  // string indexing in GIF_stream
		}
		return 1;
	}
	...
	*/
	public function __construct(array $GIF_src, array $GIF_dly, $GIF_lop, $GIF_dis, $GIF_red, $GIF_grn, $GIF_blu) {
			...
			for ($i = 0; $i < count($GIF_src); $i++) {
				...
				for ($j = (13 + 3 * (2 << (ord($this->BUF[$i][10]) & 0x07))), $k = true; $k; $j++) {
					switch ($this->BUF[$i][$j]) {  // string indexing in BUF with $j
					case '!':
	...
	private function GIFAddHeader() {
		...
		if (ord($this->BUF[0][10]) & 0x80) {
			$cmap = 3 * (2 << (ord($this->BUF[0][10]) & 0x07));  // string indexing in BUF with 10
