 ...
 *
 * @version $Id: sexystatistics.php 2012-04-05 14:30:25 svn $
 * @author 2GLux.com
...
class SexypollingModelSexyStatistics extends JModelList {
	...
	protected function getListQuery()
	{
		...
		$query->select(
				$this->getState(
						'list.select',
						'sp.id, sp.name, sp.question, sp.alias, sp.checked_out, sp.checked_out_time,sp.id_category'.
		...
		if (!empty($search)) {
			if (stripos($search, 'id:') === 0) {
				...
				$query->where('(sp.name LIKE '.$search.' OR sp.alias LIKE '.$search.')');
			}
		}
